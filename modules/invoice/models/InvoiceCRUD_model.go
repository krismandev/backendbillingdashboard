...
func GetInvoiceFromRequest(conn *connections.Connections, req datastruct.InvoiceRequest) ([]map[string]interface{}, error) {
	...
	company.city, company.phone,company.desc, company.contact_person, company.contact_person_phone FROM invoice JOIN invoice_type ON 
...
func InsertInvoice(conn *connections.Connections, req datastruct.InvoiceRequest) error {
	...
	qry := "INSERT INTO invoice (invoice_id, invoice_date,invoicestatus, company_id, month_use ,inv_type_id, last_update_date, discount_type, discount, invoice.note, created_at,created_by, printcounter,ppn, payment_method, exchange_rate_date, due_date,tax_invoice, sender, batch_id,attachment,adjustment_note,grand_total,ppn_amount) VALUES (" + baseIn + ")"
	...
	if len(req.ListInvoiceDetail) > 0 {
		...
		for _, each := range req.ListInvoiceDetail {
			...
			qryInsertDetail := "INSERT INTO invoice_detail (invoice_detail.invoice_id,invoice_detail.account_id ,invoice_detail.itemid, invoice_detail.qty,invoice_detail.adjustment, invoice_detail.uom, invoice_detail.item_price,invoice_detail.note, invoice_detail.balance_type, invoice_detail.server_id, invoice_detail.external_user_id,invoice_detail.external_username,external_sender ,last_update_username) VALUES (" + baseInDetail + ")"
...
func UpdateInvoice(conn *connections.Connections, req datastruct.InvoiceRequest) error {
	...
	_, _, errUpdateId := conn.DBAppConn.Exec("DELETE FROM invoice_detail where invoice_id=?", req.InvoiceID)
	if errUpdateId != nil {
		return errUpdateId
	}
	...
	if len(req.ListInvoiceDetail) > 0 {
		for _, each := range req.ListInvoiceDetail {
			...
			qryUpdateDetail := "INSERT INTO invoice_detail (invoice_id, account_id,qty,itemid,uom,item_price,note,balance_type,server_id,external_user_id,external_username,external_sender,last_update_username) VALUES (" + baseUpDetail + ")"
			// var invDetailParam []interface{} = []interface{}{
			// 	req.InvoiceID,
			// 	each.AccountID,
			// 	each.Qty,
...
func UpdateRevisionCounter(conn *connections.Connections, req datastruct.InvoiceRequest) error {
	...
	_, _, err = conn.DBAppConn.Exec("UPDATE invoice SET revision_counter = ? WHERE invoice_id = ? ", revisionCounter, req.InvoiceID)
	if err != nil {
		return err
	}
...
func UpdateInvoiceNo(tx *sql.Tx, countControlIdPeriod int, prefixInvoiceNo string, lastIdPeriod string, insertIdString string) error {
	...
	if countControlIdPeriod == 0 {
		...
		_, errUpdateLastIdPeriod := tx.Exec("UPDATE control_id set last_id = ? WHERE control_id.key=? AND control_id.period = ?", seqNum, "invoice_no", prefixInvoiceNo)
		...
		_, errUpdateLastIdPeriod := tx.Exec("UPDATE control_id set last_id = ? WHERE control_id.key=? AND control_id.period = ?", newLast4IdStr, "invoice_no", prefixInvoiceNo)
		if errUpdateLastIdPeriod != nil {
			tx.Rollback()
			return errUpdateLastIdPeriod
		}
